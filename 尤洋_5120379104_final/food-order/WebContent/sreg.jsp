...
<html>
  ...
  <body class="default-bg" role="document">
    ...
    <br>
    <h4>您当前的位置:	<a href="index.jsp">首页 </a>	>	注册成功</h4>
		<div class="section"> 
			<div class="page-header">注册信息</div>
			<p><h3><strong>用户名：</strong></h3><%=request.getAttribute("username") %>
			<h3><strong>密码：</strong></h3><%=request.getAttribute("password") %>
			<h3><strong>邮箱：</strong></h3><%=request.getAttribute("mail") %>
			
			<%if (((String)request.getAttribute("regauth")) == null) {} else if (((String)request.getAttribute("regauth")).equals("saler")) { %>
			<h5 style=color:red>由于您是商家，需要通过管理员审核。。。</h5><%} %>
			</p>
		</div>
