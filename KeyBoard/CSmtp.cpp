...
bool CSmtp::ConnectServer(const char *server,const unsigned short port)
{
	...
	if( (hSocket = ConnectRemoteServer(server, port)) == INVALID_SOCKET ) 
	{
	...
	if (m_pcRemoteHostName)
		delete[] m_pcRemoteHostName;
	m_pcRemoteHostName = new char[s+1];
	strcpy(m_pcRemoteHostName, server);
...
const char* const CSmtp::GetLocalHostName() 
{
	if(m_pcLocalHostName)
		delete[] m_pcLocalHostName;
	m_pcLocalHostName = new char[255];
